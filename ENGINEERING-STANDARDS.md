...
| §5.5 CSP | `every_response_carries_security_headers`, `client_script_is_external_so_csp_can_forbid_inline`, `style_src_names_the_pages_inline_stylesheet_by_hash_not_by_unsafe_inline`, `the_page_has_exactly_one_style_block` |
...
| nova crypto (X3DH, group ratchet, RLN, MPC/FROST — trust model in `session/nova*.rs`'s own module docs, not a numbered law here) | `two_clients_handshake_and_exchange_a_sealed_message`, `a_third_client_joining_mid_session_can_decrypt_a_later_broadcast`, `a_disconnected_member_is_cryptographically_excluded`, `nova_rejects_a_plaintext_message_sent_outside_the_sealed_wrapper`, `nova_rln_anonymous_post_and_double_post_slashing`, `nova_dummy_frames_are_discarded_without_a_trace`, `nova_mpc_demo_uses_a_real_distributed_operator_quorum`, `a_restarted_operator_fails_clearly_instead_of_hanging`, `complaint_is_valid_only_when_the_share_genuinely_fails_verification` |
...
| Secret scanning (gitleaks) | **Added in CI and as an opt-in local pre-commit hook** (2026-08-06) — `cargo audit`/`pnpm audit` cover known-vulnerable *dependencies*; neither one looks for a credential accidentally committed in this project's own code. `scripts/gitleaks.sh` scans full history (`--log-opts="--all"`), not just the working tree, because a leak deleted in the next commit is still in history forever unless something rewrites it out. Two confirmed-harmless matches (RFC 6455's own example WebSocket handshake key, used verbatim in tests; the literal string "ChaCha20-Poly1305") are allowlisted in `.gitleaks.toml` with the same "an exclusion without a reason is a leak being hidden" discipline `coverage-exemptions.toml` already follows. The CI job (`gitleaks/gitleaks-action@v2`) is the real gate; `scripts/install-git-hooks.sh`'s pre-commit hook is a second, opt-in line of defence that catches a leak before it's even committed, not a replacement for the CI job | Reopens if a real secret is ever found — the fix is rotating the credential and rewriting history to remove it, not just deleting it in a later commit. Extend the allowlist only for a confirmed non-secret, with the same reasoning-required discipline. |
...
| Pinning `Dockerfile.cloudflare`'s base images | **Rejected, deliberately, twice** — briefly pinned to `rust:1.97-bookworm`/`debian:bookworm-slim` for build reproducibility, reverted on explicit instruction in favour of the project's standing policy: run dependencies at latest and catch problems via `cargo audit`/`pnpm audit` (already weekly) rather than pin and periodically catch up by hand. A vulnerability scan of `rust:latest` at the time showed 1 critical and 17 high findings in the *builder* stage — discarded after `cargo build --release`, never present in what ships. Both numbers are a property of *today's* image and change on their own with the next `docker pull`; nothing in the test suite asserts on them for exactly that reason (`these_images_track_latest_on_purpose` checks the tag is a moving one, not a CVE count). The runtime stage moved again, separately, from `debian:stable-slim` to `gcr.io/distroless/cc-debian12:nonroot` — distroless, not merely slim: no shell, no package manager, nothing beyond what `ldd` against the compiled binary actually names (`libc`, `libm`, `libgcc_s` — confirmed inside a real Linux build, not assumed), and a built-in non-root user rather than one this Dockerfile has to create. `:nonroot` is a variant selector, exactly as unpinned as `latest`/`slim`/`stable`, and confirmed with `docker exec .../bin/sh` against the built image failing with "no such file or directory" | Reopens if this project's rebuild cadence ever stops being frequent enough for "latest" to mean anything — an unpinned tag only gets patched on the next build, not automatically in a container already running. A scheduled rebuild independent of code pushes (§9.4's own weekly-audit pattern, extended to the image) is the natural fix if that gap is ever observed. Separately, reopens for distroless if a future dependency needs something it does not provide — a package manager, a shell for an entrypoint script. |
...
| 100% line coverage | **Regressed to 84%, reopened by turning on `--workspace`** (2026-08-06) — `scripts/coverage.sh` ran plain `cargo tarpaulin` with no `--workspace` flag, so `nova-operator`'s own passing unit tests never counted toward the floor: the instrument disagreeing with the measurement (§0.5), not the code being untested. Fixing that surfaced how much of the `nova` MPC/RLN/DKG subsystem the coverage-instrumented run never actually exercised — the live DKG/FROST ceremony state machine in `nova-operator/src/main.rs` (`run_steady_state` was made generic over its sink, the same move `session.rs` made, and gained tests; the ceremony inside `main()` itself has not), and the STARK proof accept/duplicate/slash outcomes in `session/nova_rln.rs`, only reachable with a real proof, which only builds under `--release` (winterfell's documented debug-assertion false positive). Real tests closed part of the newly-visible gap the same day (79.78% → 85%+); both remaining pieces stay open. Previously read: **Reached, and now the floor** — 100.00%, with the same number whether or not the `#[ignore]`d tests run. Two whole-file exemptions remain and both are structural: `tests.rs` is the suite, `main.rs` is an entry point a test can never call. Previously read: **Now the target, with a reasoned exemption list** — 98.18%, every module at 100% except `session.rs` (352/373). `main.rs` was reduced to an entry point and `startup.rs` split out of it so the exclusion is honest rather than a hiding place; the test file is excluded the way `*.test.ts` is on cameronaaron.com. The 21 remaining lines are listed individually in scripts/coverage-exemptions.toml | Reopens fully at 100% once the DKG ceremony gets the same "generic over the sink" treatment `run_steady_state` already has, and the STARK-proof paths get either a debug-safe route or a merged `--release` coverage pass — see `scripts/coverage.sh`'s own comment for the itemized list. Never treat the current number as the new floor; raise it as each gap closes. The 21-line registry reopen condition below this still applies to `coverage-exemptions.toml`'s own two entries, unaffected by this regression. |
