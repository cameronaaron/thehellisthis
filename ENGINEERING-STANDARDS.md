...
| Pinned base images for `Dockerfile.cloudflare` | **Rejected, twice** — see the Docker-images row above. Distroless (`gcr.io/distroless/cc-debian12:nonroot`) replaced `debian:stable-slim` for the runtime stage: no shell, no package manager, a built-in non-root user, confirmed by attempting `docker exec .../bin/sh` against the built image and getting "no such file or directory" | Reopens if a future dependency needs something distroless does not provide (a package manager, a shell for an entrypoint script) |
